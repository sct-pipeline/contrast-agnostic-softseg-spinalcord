...
contrasts=($file_t1 $file_t1w $file_dwi_mean)
inc_contrasts=()
...
for file_path in "${inc_contrasts[@]}";do
  ...
  # Soft segmentation
  sct_process_segmentation -i ${filesoftseg}.nii.gz -vert 2,3 -vertfile ${fileseglabel}.nii.gz -o ${PATH_RESULTS}/csa_soft_GT_${contrast_seg}.csv -append 1
  
  # Soft segmentation binarized
  # Binarize output softseg for CSA computation
  sct_maths -i ${filesoftseg}.nii.gz -bin 0.5 -o ${filesoftseg}_bin.nii.gz
  ...
  rsync -avzh ${filesoftseg}.nii.gz $PATH_DATA_PROCESSED_CLEAN/derivatives/labels_softseg/${SUBJECT}/${filesoftseg}.nii.gz
