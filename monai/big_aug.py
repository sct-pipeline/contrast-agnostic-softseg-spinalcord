...
from utils import precision_score, recall_score, dice_score, check_empty_patch
from losses import SoftDiceLoss, DiceCrossEntropyLoss, AdapWingLoss
from transforms import big_aug_train_transforms, big_aug_val_transforms
from models import create_nnunet_from_plans, FCN, AHNet
...
class Model(pl.LightningModule):
    ...
    def prepare_data(self):
        ...
        transforms_train = big_aug_train_transforms(
            crop_size=self.voxel_cropping_size, 
            lbl_key='label'
        )
        transforms_val = big_aug_val_transforms(crop_size=self.inference_roi_size, lbl_key='label')
        ...
        transforms_test = big_aug_val_transforms(crop_size=self.inference_roi_size, lbl_key='label')
...
def main(args):
    ...
    elif args.model in ["ahnet"]:
        
        model2d = FCN()
        net = AHNet(num_classes=1)
        net.copy_from(model2d)
        ps = args.crop_size.split("x")
        patch_size = f"{ps[0]}x{ps[1]}x{ps[2]}"
        save_exp_id = f"{args.model}_{args.contrast}_{args.label_type}" \
                        f"_opt={args.optimizer}_lr={args.learning_rate}" \
                        f"_{args.loss}" \
                        f"_bs={args.batch_size}_{patch_size}"
    ...
    if args.loss == "DiceCE":
        ...
        loss_func = SoftDiceLoss(p=1, smooth=1e-5)
...
if __name__ == "__main__":
    ...
    # Usage:
    # python monai/main.py -m ahnet -crop 64x96x128 --contrast all --label-type hard --loss Dice -initf 32 -me 300 -bs 2 -opt adam -lr 1e-4 -cve 5 -pat 20 -epb 
    ...
    parser.add_argument('-m', '--model', choices=['unetr', 'nnunet', 'ahnet'], 
