...
root = "/home/GRAMES.POLYMTL.CA/lobouz/brainhack_data/nnUNet_data/nnUNet_raw/Dataset1_sg_basel_sci_canpr_fmri_insp_zur_agg"
...
train_subjects = [subject for subject in os.listdir(root+'/imagesTr') if subject.startswith('sg_basel_sci_canpr_fmri_insp_zur_agg_sub-')]
test_subjects = [subject for subject in os.listdir(root+'/imagesTs') if subject.startswith('sg_basel_sci_canpr_fmri_insp_zur_agg_sub-')]
logger.info(f"Total number of train subjects in the root directory: {len(test_subjects)}")
...
if args.num_cv_folds != 0:
    ...
    names_list = FoldGenerator(seed, num_cv_folds, len_data=len(train_subjects)).get_fold_names()

    for fold in range(num_cv_folds):

        train_ix, val_ix, test_ix, fold_num = names_list[fold]
        training_subjects = [train_subjects[tr_ix] for tr_ix in train_ix]
        validation_subjects = [train_subjects[v_ix] for v_ix in val_ix]
        test_subjects = [test_subjects[te_ix] for te_ix in test_ix]
    ...
    # keys to be defined in the dataset_0.json
    params = {}
    params["description"] = "sg_basel_sci_canpr_fmri_insp_zur_agg"
    params["labels"] = {
        "0": "background",
        "1": "soft-sc-seg"
        }
    ...
    params["name"] = "sg_basel_sci_canpr_fmri_insp_zur_agg"
    params["numTest"] = len(test_subjects)
    ...
    for subjects_dict in tqdm(all_subjects_list, desc="Iterating through train/val/test splits"):
        ...
        for name, subs_list in subjects_dict.items():
            ...
            for subject_no, subject in enumerate(subs_list):
                ...
                if(name == 'test'):
                    tmp_data["image"] = os.path.join(root, 'imagesTs', f"{subject}")
                    tmp_data["label"] = os.path.join(root, 'labelsTs', f"{subject.replace('_0000', '')}")

                else:
                    tmp_data["image"] = os.path.join(root, 'imagesTr', f"{subject}")
                    tmp_data["label"] = os.path.join(root, 'labelsTr', f"{subject.replace('_0000', '')}")

                if os.path.exists(tmp_data["label"]) and os.path.exists(tmp_data["image"]):
                    temp_list.append(tmp_data)
    #             temp_data_t1w = {}
    #             temp_data_t2w = {}
    #             temp_data_t2star = {}
    #             temp_data_mton_mts = {}
