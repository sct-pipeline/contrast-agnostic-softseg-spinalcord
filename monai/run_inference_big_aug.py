...
from transforms import big_aug_val_transforms
from utils import precision_score, recall_score, dice_score
from models import AHNet, FCN

# NOTE: for running inference with gpu, "cupy" package is required (pip install cupy)
...
# DATA
# --------------------------------
def prepare_data(root, dataset_name="spine-generic", crop_size=(64, 192, -1)):
    # set deterministic training for reproducibility
    # set_determinism(seed=self.args.seed)

    # load the dataset
    dataset = os.path.join(root) #, f"datasplit_{dataset_name}_seed50.json")
    test_files = load_decathlon_datalist(dataset, True, "test")
    print(f"Number of test files: {len(test_files)}")

    if args.debug:
        test_files = test_files[:48]

    # define test transforms
    transforms_test = big_aug_val_transforms(crop_size=crop_size, lbl_key='label')
...
def main(args):
    ...
    # get the last folder name from the checkpoint path
    ...
    # define inference patch size same as the crop size
    crop_size = tuple([int(i) for i in args.crop_size.split("x")])
    # defin inference patch size same as the one in training
    # inference_roi_size = (int(patch.split("x")[0]), int(patch.split("x")[1]), int(patch.split("x")[2]))
    inference_roi_size = (128, 192, 160)

    # define the dataset and dataloader
    test_ds, test_post_pred = prepare_data(dataset_root, dataset_name, crop_size=crop_size)
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # # binarize the predictions at 0.5
            # pred = (pred > 0.5).float()
        ...
        avg_metrics = {
            ...
            "std_dice_binary": round(std_hard_dice_test, 2),
        ...
        logger.info(f"Test (Hard) Dice Std: {std_hard_dice_test}")
