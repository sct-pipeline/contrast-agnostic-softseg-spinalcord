...
contrasts=("all") # We can't differentiate between variants of MTS
...
do
  ...
  do
    ...
    do
      ...
      mkdir $output_dir
      #echo ./batch_configs/"$config"_"$contrast"_seed="$seed".json
      CUDA_VISIBLE=2 ivadomed --train -c ./batch_configs/group-9/"$config"_"$contrast"_seed="$seed".json --path-output $output_dir
    done
  done
done