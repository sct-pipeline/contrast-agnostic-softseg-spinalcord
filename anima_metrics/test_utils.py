...
def fetch_filename_details(filename_path):
    ...
    session = re.findall(r'ses-..', filename_path)        # only gets the 2 characters after ses- (e.g., 01)
    sessionID = session[0] if session else ""               # Return None if there is no session
    # NOTE: uncomment these lines when running inference on sct_deepseg_lesion dataset (cuz it's not BIDS-compliant)
    # session = re.search('ses-(.*?)[_/]', filename_path)
    # sessionID = session.group(0)[:-1] if subject else ""    # [:-1] removes the last underscore or slash
